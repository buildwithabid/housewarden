...
| `npm run e2e` | `tsx tests/e2e/run.ts`: start `next dev -p 3123` with a temp data dir and known token; wait for `/api/mcp` to answer 401; then with `@modelcontextprotocol/client` + `StreamableHTTPClientTransport`: initialize (legacy and auto) → tools/list = 31 → `get_household_summary` → `add_shopping_item` (executed) → `mark_bill_paid` (needs_confirmation) → `confirm_action` (executed) → `confirm_action` again (idempotent_replay) → `verify_audit_chain` (intact) → raw fetch with `Origin: http://evil.test` → 403 → raw fetch with bad token → 401. Print a pass/fail table; exit code = failures. Always kill the server it started. |
